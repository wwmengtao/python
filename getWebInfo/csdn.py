...
def write_excel_xls_append(path, value):
 ...
 # path指的是excel文件的路径;value指的是数据,是一个嵌套列表
def write_excel_xls_append2(path, sheetName, value):
    """向excel表中增加数据"""
    index = len(value)  # 获取需要写入数据的行数
    workbook = xlrd.open_workbook(path)  # 打开工作簿
    worksheet = workbook.sheet_by_name(sheetName)  # 获取工作簿中名称为sheetName的表格
    rows_old = worksheet.nrows  # 获取表格中已存在的数据的行数
    new_workbook = copy(workbook)  # 将xlrd对象拷贝转化为xlwt对象
    new_worksheet = new_workbook.get_sheet(0)  # 获取转化后工作簿中的第一个表格
    for i in range(0, index):
...
for index in range(1, page_num + 1):
    ...
    for i in range(0, len(csdn_article_title_list)):
        ...
        write_excel_xls_append2(file_name, author_name, data)
